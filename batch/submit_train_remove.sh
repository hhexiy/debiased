...
#for seed in 9 99; do
    for model_name in book_corpus_wiki_en_uncased; do
        rremove=0
        for oremove in 0.001 0.004 0.016 0.064; do
            ## bert base
            ...
            ## bert large
            sbatch --job-name bert-$task_dir-$model_name --export=command="make train-bert-large task=$task test-split=$test_split nepochs=10 model_name=$model_name bert=bertl exp=$task_dir/bert-large seed=$seed overlap=$oremove random=$rremove" batch/run.sh
            ## roberta base
            ...
            ## roberta large
            sbatch --job-name roberta-$task_dir-$model_name --export=command="make train-bert-large task=$task test-split=$test_split nepochs=10 bert=robertal model_name=openwebtext_ccnews_stories_books_cased exp=$task_dir/roberta-large seed=$seed overlap=$oremove random=$rremove" batch/run.sh
        done
    done
done
